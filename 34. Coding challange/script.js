...
const ownersEatToMuch = dogs.filter(dog => dog.curFood >= dog.recFood).map(dog => dog.owners).flat()
const ownersEatToLittle = dogs.filter(dog => dog.curFood <= dog.recFood).map(dog => dog.owners).flat()
...
const total = dogs.filter(dog => dog.curFood > dog.recFood *0.90 && dog.curFood > dog.recFood *1.10).length
console.log(total);
